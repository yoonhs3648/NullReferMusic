-- 검색 규칙: Gemini google_search / Groq web_search — 도구가 요청에 있을 때만.

UPDATE public."LLMSystemPrompt"
SET
  "Content" =
    E'웹 검색 도구(google_search 또는 web_search)가 **이번 요청에 실제로 포함될 때만** 검색한다. 도구가 없으면 function/tool call을 하지 말고 텍스트로만 답한다.\n'
    ...
    || E'앱 다운로드 실행(플랫폼 선택·곡 검색·다운로드 시작)에는 웹 검색 도구를 쓰지 않는다.\n'
    || E'검색이 필요 없는 예: 일반·분위기·장르 추천, 유명한 곡/가수 소개, 용어·이론·악기 설명, 가사 해석, 음악사, 앱 기능 안내(다운로드 실행 요청이 아닌 경우).',
  "UpdateDate" = now(),
