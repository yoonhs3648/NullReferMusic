-- Failure list includes job_status so the admin UI can prefix
-- (dead)/(quarantined)/(blocked) on the red reason line.
...
begin
  ...
  select jsonb_build_object(
    'items', coalesce((
      ...
      from (
        select f.job_id, f.artist, f.title, f.error_message, f.job_status, f.created_at
        from failed f
...
comment on function public.music_rpc_admin_schedule_run_failures(text, uuid, integer, integer) is
  '스케줄 실행의 실패 곡: 가수·제목·job_status·대표 실패 메시지. dead/quarantined/blocked';
