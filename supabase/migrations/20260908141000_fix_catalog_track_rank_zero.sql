-- Last.fm @attr.rank가 0이면 catalog 후보 INSERT가 ck_music_catalog_track_candidate_rank(1..5000)에 걸려 400(23514)이 난다.
...
set failure_count = greatest(0, r.failure_count - 1)
where r.run_status = 'running'
  and exists (
    select 1
    from public.music_sync_job j
    where j.schedule_run_id = r.schedule_run_id
      and j.job_kind = 'lastfm_track_pool'
      and j.job_status = 'quarantined'
      and coalesce(j.last_error_message, '') ilike '%next_page%'
  );

update public.music_sync_job j
set job_status = 'retry',
    available_at = now(),
