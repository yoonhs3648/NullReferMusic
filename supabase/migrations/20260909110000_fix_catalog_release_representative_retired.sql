-- Catalog apply must satisfy ck_music_release_representative_retired.
-- Extra editions keep the existing representative and set retired_at.
...
comment on function public.music_rpc_apply_catalog_recording_bundle(uuid, uuid, jsonb) is
  '차트 Recording 최소 bundle을 원장에 넣는다. 이미 대표 Release가 있으면 추가 에디션은 retired_at을 넣어 비대표로 저장한다';
