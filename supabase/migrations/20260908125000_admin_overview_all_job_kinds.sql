-- 실행 탭이 music_schedule_run만 봐서 retention 이력이 안 보이던 것을 고친다.
-- Last.fm 503 폭주 재시도는 Edge worker에서 따로 완화한다.

grant usage on schema public to nrm_music_rpc_owner;
grant select on table public.nrm_system_schedule_run to nrm_music_rpc_owner;
...
comment on function public.music_rpc_admin_overview(text, integer, integer) is
  '관리 UI 개요. 실행/실패 탭은 수집 music_schedule_run과 retention nrm_system_schedule_run을 같이 반환한다.';

comment on table public.nrm_system_schedule_run is
  '시스템 스케줄 실행 이력(수집 + retention). 실행/실패 탭은 수집과 retention을 같이 보여 준다.';
