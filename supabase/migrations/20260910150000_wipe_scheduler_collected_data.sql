-- One-shot: force-stop in-flight collection and wipe scheduler-inserted music data.
-- Keeps schedule definitions. Restores previous is_enabled, but next_run_at is
-- advanced to the next slot so Cron does not immediately reclaim an empty DB.
...
create or replace function public.music_tmp_wipe_scheduler_collected_data()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_n bigint;
  v_music_enabled uuid[];
  v_system_enabled uuid[];
begin
  perform pg_catalog.set_config('nrm.music_capacity_purge', 'on', true);

  select coalesce(array_agg(schedule_id), '{}')
    into v_music_enabled
  ...
  where is_enabled and job_kind = 'musicbrainz_collection';
  ...
  set next_run_at = public.nrm_system_schedule_compute_next_run(
      ...
      claim_fence_token = null,
      claimed_by = null,
      is_enabled = schedule_id = any(v_music_enabled);

  update public.nrm_system_schedule s
  set is_enabled = s.schedule_id = any(v_system_enabled),
      next_run_at = coalesce(m.next_run_at, s.next_run_at),
      updated_at = now()
  ...
  update public.nrm_system_schedule
  set is_enabled = true
  where schedule_id = any(v_system_enabled)
    and job_kind = 'musicbrainz_collection'
    and not exists (
      select 1
      from public.music_collection_schedule m
      where m.schedule_id = nullif(nrm_system_schedule.config->>'music_schedule_id', '')::uuid
    );
...
revoke all on function public.music_tmp_wipe_scheduler_collected_data()
  from public, anon, authenticated;
grant execute on function public.music_tmp_wipe_scheduler_collected_data()
  to postgres, service_role;

alter function public.music_tmp_wipe_scheduler_collected_data()
  owner to nrm_music_rpc_owner;

select public.music_tmp_wipe_scheduler_collected_data() as wipe_counts;

drop function public.music_tmp_wipe_scheduler_collected_data();
