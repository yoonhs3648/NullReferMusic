...
<#
.SYNOPSIS
  facebook/wav2vec2-base-960h ONNX export + FinDIT-Studio HF 업로드.

.DESCRIPTION
  1) library/wav2vec2-base-en/model.onnx 생성 (FP32, ~380MB)
  2) HF_TOKEN 환경 변수로 FinDIT-Studio/wav2vec2-base-english-onnx 업로드
...
$RepoId = 'FinDIT-Studio/wav2vec2-base-english-onnx'

if (-not $SkipExport) {
    & (Join-Path $PSScriptRoot 'Export-Wav2Vec2EnOnnx.ps1')
}

if (-not (Test-Path $ModelOut)) {
    Write-Error "model.onnx 없음: $ModelOut — Export-Wav2Vec2EnOnnx.ps1 먼저 실행"
}
...
HF_TOKEN 환경 변수 또는 library\.hf-token.local 파일이 필요합니다 (FinDIT-Studio 쓰기 권한).
  ...
  .\scripts\Publish-Wav2Vec2EnOnnx.ps1
"@
}
...
api.upload_file(
    ...
    repo_id=repo_id,
    repo_type="model",
    commit_message="Upload wav2vec2-base-960h ONNX for NullReferMusic FA",
)
