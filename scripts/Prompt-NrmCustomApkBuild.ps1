...
function Read-ValidatedGithubPat {
    while ($true) {
        Write-Host ""
        Write-Host "GitHub PAT is required for this release APK build."
        Write-Host "  - Used to read/write GitHub data/*.json during the build."
        Write-Host "  - Repo: $NrmGithubRepo (token needs repo contents read/write scope)."
        if (Test-Path -LiteralPath $SecretsPath) {
            Write-Host "  - A saved PAT exists at .secrets\nrm-github-data.pat; enter a valid token to continue."
        }
        $line = Read-Host 'GitHub PAT (ghp_... or github_pat_...)'
        if (-not (Test-GithubPatFormat $line)) {
            Write-Host 'WARNING: Invalid format. Enter ghp_... or github_pat_...' -ForegroundColor Yellow
            continue
        }
        ...
        Write-Host 'WARNING: Invalid or unauthorized PAT. Check token scope (repo) and try again.' -ForegroundColor Yellow
    }
}
...
function Test-CustomSerialNumber {
    param([string]$Raw)
    return (Test-SafeCustomField $Raw) -and (Test-SerialNotReserved $Raw)
}
...
function Read-ValidatedLine {
    ...
    while ($true) {
        ...
        $line = Read-Host -Prompt $InputPrompt
        if (& $Validator $line) {
            return $line.Trim()
        }
...
if ($doCustom -notmatch '^(?i)[YN]$') {
    Write-Host ""
    Write-Host 'Please enter Y or N.' -ForegroundColor Red
    exit 1
}

[void](Read-ValidatedGithubPat)

if ($doCustom -match '^(?i)Y$') {
    $appName = Read-ValidatedLine -InputPrompt 'app name (two words, one space)' -Hint 'Launcher / logo name - exactly two words separated by a single space (e.g. Hyun Music).' -Validator ${function:Test-TwoWordAppName} -InvalidMessage 'Invalid: enter exactly two words with one space between them.'

    $userName = Read-ValidatedLine -InputPrompt 'user name' -Hint 'Up to 30 characters. No quotes or JSON brackets { } [ ].' -Validator ${function:Test-SafeCustomField} -InvalidMessage 'Invalid: max 30 chars, no ", [, ], {, } or control characters.'

    $serialNo = Read-ValidatedLine -InputPrompt 'product serial number' -Hint 'Stored in APK as SerialNo (not shown in version UI). Same character rules as user name. "Admin" is reserved.' -Validator ${function:Test-CustomSerialNumber} -InvalidMessage 'Invalid: max 30 chars, no quotes/brackets, and "Admin" is reserved for system use.'
