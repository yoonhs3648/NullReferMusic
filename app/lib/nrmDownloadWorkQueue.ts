...
import { resetInnertubeExtractSessionOnQueueIdle } from '@/lib/nrmInnertubeExtractSession';
...
async function pumpLoop(): Promise<void> {
  while (audioQueue.length > 0 || lyricsQueue.length > 0) {
    ...
    if (lyricsQueue.length > 0) {
      logQueue('lyrics_lane_resume', {
        lyricsPending: lyricsQueue.length,
        nextId: lyricsQueue[0]?.id,
        nextWaitMs: lyricsQueue[0]
          ? Date.now() - lyricsQueue[0].enqueuedAt
          : 0,
      });
  ...
  logQueue('lanes_idle', {});
  resetInnertubeExtractSessionOnQueueIdle();
}
...
function enqueue(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (lane === 'lyrics' && audioQueue.length > 0) {
      logQueue('lyrics_deferred', {
        id,
        label,
        audioPending: audioQueue.length,
      });
