...
/** 웹(개발): PC Spring 백엔드 `library/whisper` — APK와 동일 다운로드 UX */
...
export async function startWhisperModelDownloadOnDevice(modelId: NrmWhisperModelId): Promise<void> {
  if (!isWhisperModelNativeAvailable()) return;
  const base = await getResolvedApiBaseUrl();
  if (!base) return;
  ensurePollLoop();
...
export function subscribeWhisperModelDownloadEvents(onEvent: DownloadListener): () => void {
